...
export class RealFileLoaderService {
  ...
  async getSingleFile(filePath: string): Promise<File> {
    ...
    const fileEntry: FileEntry = (await this.ionicFileService.resolveLocalFilesystemUrl(
      filePath
    )) as FileEntry;

    // Get File from FileEntry. Again note that this file does not contain the actual file data yet.
    const cordovaFile: IFile = await this.convertFileEntryToCordovaFile(
      fileEntry
    );
  ...
  private convertCordovaFileToJavascriptFile(
    cordovaFile: IFile
  ): Promise<File> {
    return new Promise<File>((resolve, reject) => {
      ...
      reader.onloadend = () => {
        if (reader.error) {
          ...
          const blob: any = new Blob([reader.result], {
